...
individual-level fit for student loan repayments; the fact that the
...
is a genuinely external comparator there; the \frs{} instrument's
inability to express five of the ten \spi{} source constituents, which
fences the entire \hmrc{} distribution-table replay
(\ref{sec:validation-hmrc-replay}); and the promotion adjudication's own
recorded finding that unqualified superiority over the incumbent dataset
is not supported, with four named metrics on which both artifacts miss
badly (\ref{sec:validation-promotion})). State each limitation plainly
