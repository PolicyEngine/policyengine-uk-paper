...
\begin{enumerate}
    ...
    administrative totals -- and the versioned, release-gated data
    pipeline that produces it, including the input-coverage contract every
    release candidate must pass (Section~\ref{sec:data}).
    ...
    individual-level calculations against \hmrc{}'s \spi{} microdata; it
    also documents two data-level validation records from the \populace{}
    pipeline -- the adjudicated promotion of the current default dataset
    and a fenced replay register against \hmrc{} distribution tables
